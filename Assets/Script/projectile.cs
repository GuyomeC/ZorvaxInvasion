...
public class projectile : MonoBehaviour {
    ...
    void OnTriggerEnter2D(Collider2D truc) {
        if (truc.tag == "Ennemi") {                 // Si le truc qu'on touche a le tag "Ennemi"
            Vector3 spawnButin = new Vector3(transform.position.x + 2, transform.position.y, transform.position.z);
            Quaternion spawnButinRot = Quaternion.identity;
            truc.GetComponent<EnemyStats>().TakeDamage(degats, spawnButin, spawnButinRot); // On cherche sur lui une fonction qui s'appel "takeDamage", et on la lance en lui donnant le nombre de dégat qu'on fait
